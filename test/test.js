...
test('BitFields: tuple descriptor support', () => {
    const bitFields = bin.bitfields.BitFields(0, [4, 4]);
    const result = bitFields.to(0xAB);
...
// Async operations
//=============================================================================
async function openFile(filename, flags = fs.constants.O_RDWR | fs.constants.O_CREAT | fs.constants.O_TRUNC) {
    const fd = await fs.open(filename, flags);
    return new bin.async.stream((offset, data) => fd.read(data, 0, data.length, offset).then(read => read.bytesRead), (offset, data) => fd.write(data, 0, data.length, offset).then(_write => undefined), _s => fd.close());
}
const asyncTests = [
