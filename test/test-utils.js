...
const float = __importStar(require("../dist/utilities/float"));
const bit = __importStar(require("../dist/bit"));
const bitfields = __importStar(require("../dist/utilities/bitfields"));
const typedArray = __importStar(require("../dist/utilities/typedArray"));
function assertClose(label, got, expected, rel = 1e-12, abs = 1e-12) {
...
for (const [x, y] of [
    ...
    const got = +float.float16(x).pow(float.float16(y));
    const expected = x ** y;
...
for (const [x, y] of [
    ...
    const got = +float.float128(x).pow(float.float128(y));
    const expected = x ** y;
...
for (const i of f16)
    console.log(+i);
const StructArray = typedArray.BitFields(bitfields.BitFields(0, { a: 1, b: 2, c: 3 }));
