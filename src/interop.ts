...
export function read<T extends sync.TypeReader|async.TypeReader>(s: _stream, spec: T, obj?: any) : MaybePromise<ReadType<T>>;
...
export function write(s: _stream, type: sync.TypeWriter|async.TypeWriter, value: any) : MaybePromise<void>;
...
export function write_merge(s: sync._stream, type: sync.TypeWriter) : void;
export function write_merge(s: async._stream, type: async.TypeWriter) : Promise<void>;
export function write_merge(s: _stream, type: sync.TypeWriter|async.TypeWriter) : MaybePromise<void>;
export function write_merge(s: any, type: sync.TypeWriter|async.TypeWriter) : MaybePromise<void> {
	const value = s.obj;
	if (isWriter(type))
		return type.put(s, value);
...
export function readn<T extends sync.TypeReader|async.TypeReader>(s: _stream, type: T, n: number) : MaybePromise<ReadType<T>[]>;
export function readn<T extends sync.TypeReader|async.TypeReader>(s: any, type: T, n: number) : MaybePromise<ReadType<T>[]> {
	...
	return after(acc, () => s.popObj(result));
}
...
export function makex<T extends object | number | bigint | string | boolean>(type: TypeX<T>): TypeTX<T, T>;
export function makex<T extends object | number | bigint | string | boolean, T2>(type: TypeX<T>, discriminator: (v2: T2) => T | undefined): TypeTX<T, T2>;
export function makex<T extends object | number | bigint | string | boolean, T2>(type: TypeX<T>, discriminator?: (v2: T2) => T | undefined): any {
	...
	if (typeof type === 'string') {
		return {
			get: (s: any) => s.lookupObj(type),
			put: (s: any) => s.lookupObj(type)
		};
	}
